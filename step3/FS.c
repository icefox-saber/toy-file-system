...
int format_handle(char *params)
{
    ...
    { // 初始化inode table
        if (init_inode(&inode_table[i], i, 0, 0, 0, 65535) < 0||write_inode(&inode_table[i], i)<0)
        {
...
int mk_handle(char *params)
{
    ...
    add_to_dir_inode(&inode_table[current_dir], params, 0);
    return 0;
}
...
int mkdir_handle(char *params)
{
    ...
    { // 重名
        ...
        return 0;
    }add_to_dir_inode(&inode_table[current_dir], params, 1);
    return 0;
}
...
int rm_handle(char *params)
{
    ...
    rm_from_dir_inode(&inode_table[current_dir], params);
    return 0;
}
...
int cd_handle(char *params)
{
    ...
    uint16_t ori_dir = current_dir; // 保存当前目录ID和路径字符串
    ...
    while (path != NULL)
    {
        ...
        { // . 代表当前目录，无视之
            ...
            if (current_dir != 0 && strcmp(cur_path_string, "/") != 0)
